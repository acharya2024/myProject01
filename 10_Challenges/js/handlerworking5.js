document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadQuestions(data) {
        ...
        data.questions.forEach((question, index) => {
            ...
            magnifierButton.textContent = "Magnify";
            magnifierButton.classList.add("btn", "btn-info", "btn-sm", "mt-2", "me-2");
           
          

            magnifierButton.onclick = function (event) {
                const listItem = this.closest('.list-group-item');
                listItem.classList.toggle('fullscreen-question');
                if (listItem.classList.contains('fullscreen-question')) {
                    // When full screen is activated:
                    document.body.style.overflow = 'hidden'; // Prevent scrolling on the body
                    document.querySelector('.container').style.display = 'none'; // Hide other content
                } else {
